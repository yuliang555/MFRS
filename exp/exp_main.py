...
from models import Informer, Autoformer, Transformer, DLinear, Linear, NLinear, PatchTST, SegRNN, CycleNet, \
    LDLinear, SparseTSF, RLinear, RMLP, CycleiTransformer, MFRS
...
class Exp_Main(Exp_Basic):
    ...
    def vali(self, vali_loader, criterion):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_cycle, batch_rs) in enumerate(vali_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if any(substr in self.args.model for substr in {'MFRS'}):
                            outputs = self.model(batch_x, batch_rs)
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            # max_memory = 0
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_cycle, batch_rs) in enumerate(train_loader):
                iter_count += 1
    ...
    def test(self, setting, test=0):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark, batch_cycle, batch_rs) in enumerate(test_loader):
                batch_y = batch_y.float().to(self.device)
                               
                if self.args.use_align:
                    batch_rs = test_data.synchronous_alignment.alignment(batch_x, self.device)
                    batch_x = batch_x.float().to(self.device)                    
                else:
                    batch_rs = batch_rs.float().to(self.device)
                    batch_x = batch_x.float().to(self.device)
        ...
        metric_path = f"results_{self.args.pred_len}.csv"
        if not os.path.exists(metric_path):
            open(metric_path, "w").close()                      
        df = pandas.DataFrame({
            "settings": [f"{setting}"],
            "align": [f"True_L={self.args.L}_N={self.args.N}" if self.args.use_align else "False"],
            "MSE": [mse],
            "MAE": [mae],
        })
